...
# =================================================================== #
# Session tests — create / list / revoke                              #
# =================================================================== #

class TestSessions:
    """Verify session tracking and revocation."""

    def _login(self, client):
        with client.session_transaction() as sess:
            sess["user_id"] = 1
            sess["user_name"] = "Demo User"
            sess["session_token"] = "test-token-1"

    def test_create_session(self, db):
        """create_session should insert a row and return its id."""
        sid = db.create_session(1, "token-abc", "Chrome/Windows", "127.0.0.1")
        assert sid > 0
        sessions = db.get_user_sessions(1)
        assert len(sessions) == 1
        assert sessions[0]["token"] == "token-abc"
        assert sessions[0]["user_agent"] == "Chrome/Windows"
        assert sessions[0]["ip_address"] == "127.0.0.1"

    def test_get_user_sessions_scoped_to_user(self, db):
        """Sessions for user A must never appear for user B."""
        user2_id = db.create_user("Second", "second@example.com")
        db.create_session(1, "token-1", "Chrome", "1.1.1.1")
        db.create_session(user2_id, "token-2", "Firefox", "2.2.2.2")
        sessions1 = db.get_user_sessions(1)
        sessions2 = db.get_user_sessions(user2_id)
        assert len(sessions1) == 1
        assert sessions1[0]["token"] == "token-1"
        assert len(sessions2) == 1
        assert sessions2[0]["token"] == "token-2"

    def test_revoke_session_scoped_to_user(self, db):
        """revoke_session must only delete the owning user's session."""
        user2_id = db.create_user("Second", "second@example.com")
        sid1 = db.create_session(1, "token-1", "Chrome", "1.1.1.1")
        sid2 = db.create_session(user2_id, "token-2", "Firefox", "2.2.2.2")
        # User 1 tries to revoke user 2's session — should fail.
        assert db.revoke_session(1, sid2) is False
        # User 1 revokes their own session — should succeed.
        assert db.revoke_session(1, sid1) is True
        assert db.get_user_sessions(1) == []
        assert len(db.get_user_sessions(user2_id)) == 1

    def test_delete_session_by_token(self, db):
        """delete_session_by_token should remove the row by token."""
        db.create_session(1, "token-abc", "Chrome", "1.1.1.1")
        assert db.delete_session_by_token("token-abc") is True
        assert db.get_user_sessions(1) == []

    def test_settings_page_renders_sessions(self, client, db):
        """GET /settings should render the user's real sessions."""
        self._login(client)
        db.create_session(1, "test-token-1", "Chrome on Windows", "127.0.0.1")
        db.create_session(1, "test-token-2", "Firefox on Mac", "10.0.0.1")
        resp = client.get("/settings")
        assert resp.status_code == 200
        html = resp.get_data(as_text=True)
        assert "Chrome on Windows" in html
        assert "Firefox on Mac" in html
        assert "This device" in html  # current session marked

    def test_revoke_session_route(self, client, db):
        """POST /settings/sessions/revoke should revoke a session."""
        self._login(client)
        sid = db.create_session(1, "test-token-2", "Firefox on Mac", "10.0.0.1")
        resp = client.post("/settings/sessions/revoke", data={"session_id": str(sid)})
        assert resp.status_code == 302
        assert db.get_user_sessions(1) == []

    def test_revoke_session_requires_auth(self, client):
        """POST /settings/sessions/revoke without auth should redirect."""
        resp = client.post("/settings/sessions/revoke", data={"session_id": "1"})
        assert resp.status_code == 302
        assert "/login" in resp.headers["Location"]

    def test_revoke_session_cannot_revoke_other_users(self, client, db):
        """A user must not be able to revoke another user's session."""
        user2_id = db.create_user("Second", "second@example.com")
        sid2 = db.create_session(user2_id, "token-2", "Firefox", "2.2.2.2")
        self._login(client)
        resp = client.post("/settings/sessions/revoke", data={"session_id": str(sid2)})
        assert resp.status_code == 302
        # User 2's session must still exist.
        assert len(db.get_user_sessions(user2_id)) == 1

    def test_revoke_session_invalid_id(self, client):
        """Invalid session id should flash an error and not crash."""
        self._login(client)
        resp = client.post("/settings/sessions/revoke", data={"session_id": "abc"})
        assert resp.status_code == 302
